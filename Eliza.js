.pragma library
// omarchy-eliza: shared trace ceiling for the desktop service.
// ELIZA engine — verbatim from Anthony Hay's eliza.html (https://github.com/anthay/ELIZA,
// src/eliza.html v1.00, the pure-JS section before the browser console code).
// "ELIZA implementation by Ant & Max Hay, 2023 (CC0 1.0) Pub Domain".
// Local changes are marked with `// omarchy-eliza:` comments and listed in docs/ENGINE.md.
...
function utf8ArrayFromString(s) {
    // omarchy-eliza: hand-rolled UTF-8 encoder; QML JS (and Node's vm) has no TextEncoder
    const result = [];
    for (const c of utf32ArrayFromString(s)) {
        if (c < 0x80) result.push(c);
        else if (c < 0x800) result.push(0xC0 | (c >> 6), 0x80 | (c & 0x3F));
        else if (c < 0x10000) result.push(0xE0 | (c >> 12), 0x80 | ((c >> 6) & 0x3F), 0x80 | (c & 0x3F));
...
function match(tags, p, w) {
    // omarchy-eliza: charge recursive steps before copying arrays; unwind on exhaustion.
    if (tags.matchBudget && tags.matchBudget.remaining-- <= 0) { tags.matchBudget.exhausted = true; return [false]; }
    let pattern = p.slice();
    let words = w.slice();
    ...
    if (n < 0) { // patword is e.g. "ARE" or "(*SAD HAPPY DEPRESSED)"
        ...
        let component = [];
        let mc;
        let success; // omarchy-eliza: was an implicit global (sloppy mode); QML rejects the write
        while (true) {
            ...
            // omarchy-eliza: do not keep exploring wildcard alternatives after exhaustion.
            if ((tags.matchBudget && tags.matchBudget.exhausted) || words.length === 0) {
                return [false];
            }
...
function hash(d, n) {
    ...
    // omarchy-eliza: BigInt is not available in QML JS, so the 72-bit square is
    // computed exactly with 18-bit limbs in doubles. d is a 36-bit Number.
    d = d % 0x800000000; // d &= 0x7FFFFFFFF (36 bits; % is exact on non-negative integers)
    const a = Math.floor(d / 0x40000), b = d % 0x40000; // d = a*2^18 + b, a,b < 2^18
    const lo = b * b, mid = 2 * a * b, hi = a * a;      // d*d = hi*2^36 + mid*2^18 + lo
    // normalise into base-2^18 digits D0..D3 so every partial sum stays < 2^53
    let D0 = lo % 0x40000, carry = Math.floor(lo / 0x40000);
    let t1 = mid + carry; let D1 = t1 % 0x40000; carry = Math.floor(t1 / 0x40000);
    let t2 = hi + carry;  let D2 = t2 % 0x40000; let D3 = Math.floor(t2 / 0x40000);
    const shift = 35 - Math.floor(n / 2); // 28..35, so 36 - shift >= 1 and the top terms stay integral
    const q = D3 * Math.pow(2, 54 - shift) + D2 * Math.pow(2, 36 - shift)
            + Math.floor((D1 * 0x40000 + D0) / Math.pow(2, shift));
    return q % Math.pow(2, n);
}
...
function lastChunkAsBcd(str) {
    let result = 0; // omarchy-eliza: plain Number, a 36-bit value is exact in a double (no BigInt in QML JS)

    const append = (u8) => {
        result = result * 64 + (hollerithDefined(u8) ? hollerithEncoding[u8] : (u8 & 0x3F));
    };
...
class RuleMemory {
    ...
    createMemory(keyword, words, tags) {
        ...
        // omarchy-eliza: retain only the newest 64 memories.
        if (this.memories.length >= 64) this.memories.shift();
...
class Eliza {
    ...
    response(input) { // omarchy-eliza: synchronous (was async; QML has no setTimeout and the UI does not need yielding)
        // omarchy-eliza: one shared budget for memory and all keyword decompositions.
        this.tags.matchBudget = {remaining: 10000, exhausted: false};
        ...
        while (keystack.length > 0) {
            // omarchy-eliza: removed `await delay()` and the browser interrupt check
            ...
            // omarchy-eliza: exhausted matching falls through to NONE, never NOMATCH or an exception.
            if (this.tags.matchBudget.exhausted) break;
        ...
        // omarchy-eliza: reserve 1000 steps for NONE independently of the spent keyword budget.
        this.matchSteps = 10000 - Math.max(0, this.tags.matchBudget.remaining);
        this.matchBudgetExceeded = this.tags.matchBudget.exhausted;
        if (this.matchBudgetExceeded) words = [];
        this.tags.matchBudget = {remaining: 1000, exhausted: false};
        const noneRule = this.rules.get(SPECIAL_RULE_NONE);
        const [ action, response ] = noneRule.applyTransformation(words, this.tags);
        // omarchy-eliza: malformed/exhausted fallback patterns also return a safe reply.
        if (action !== ACTION_COMPLETE) return "PLEASE GO ON";
...
class Tokenizer {
    ...
    peektok() {
        // omarchy-eliza: rewritten without `return this.t = ...` (Qt V4 quirk, see docs/ENGINE.md)
        if (!this.got_token) {
            this.t = this.readtok();
            this.got_token = true;
        }
...
function testElizaUppercase() {
    ...
    console.assert(elizaUppercase("Æmilia, æsop & Phœbë") === "ÆMILIA, ÆSOP & PHŒBË");
    console.assert(elizaUppercase("à â ç é è ê ë î ï ô ù û ü ÿ æ œ") === "À Â Ç É È Ê Ë Î Ï Ô Ù Û Ü Ÿ Æ Œ");
    console.assert(elizaUppercase("Maroš Šefčovič") === "MAROŠ ŠEFČOVIČ");
    console.assert(elizaUppercase("Сайфи Кудаш Гилемдар Зигандарович") === "САЙФИ КУДАШ ГИЛЕМДАР ЗИГАНДАРОВИЧ");
    console.assert(elizaUppercase("¡pónk!") === " PÓNK.");
    console.assert(elizaUppercase("I'm not entirely certain—maybe the nervousness was there before–") === "I'M NOT ENTIRELY CERTAIN,MAYBE THE NERVOUSNESS WAS THERE BEFORE,");

    // 'RIGHT SINGLE QUOTATION MARK' (U+2019)
    console.assert(elizaUppercase("I’m depressed") === "I'M DEPRESSED");
...
function testHash() {
    console.assert(hash(parseInt("214366217062", 8),  7) === 14);
    console.assert(hash(parseInt("302551256060", 8),  2) === 3);
    console.assert(hash(parseInt("423124626060", 8),  2) === 1);
    console.assert(hash(parseInt("633144256060", 8),  2) === 0);
    console.assert(hash(parseInt("777777777777", 8),  7) === 0x70);
    console.assert(hash(parseInt("777777777777", 8), 15) === 0x7F00);
    console.assert(hash(0x555555555,    15) === 0x46E3);
    console.assert(hash(0xF0F0F0F0F,    15) === 0x7788);
    console.assert(hash(parseInt("214366217062", 8), 15) === 0x70EE);
    console.assert(hash(parseInt("633144256060", 8), 15) === 0x252E);
    console.assert(hash(0,               7) === 0);
...
function testLastChunkAsBcd() {
    console.assert(lastChunkAsBcd("")               === parseInt("606060606060", 8));
    console.assert(lastChunkAsBcd("X")              === parseInt("676060606060", 8));
    console.assert(lastChunkAsBcd("HERE")           === parseInt("302551256060", 8));
    console.assert(lastChunkAsBcd("ALWAYS")         === parseInt("214366217062", 8));
    console.assert(lastChunkAsBcd("INVENTED")       === parseInt("252460606060", 8));
    console.assert(lastChunkAsBcd("123456ABCDEF")   === parseInt("212223242526", 8));
    console.assert(lastChunkAsBcd("Ç")              === parseInt("030760606060", 8));
}
...
function testDoctor() { // omarchy-eliza: synchronous
...
`;
    ...
    let response; // omarchy-eliza: was an implicit global (sloppy mode); QML rejects the write
    for (let i = 0; i < CACM_1966_CONVERSATION.length; i += 2) {
        response = eliza.response(CACM_1966_CONVERSATION[i]);
    ...
    for (let i = 0; i < imaginedContinuation2023.length; i += 2) {
        response = eliza.response(imaginedContinuation2023[i]);
...
// omarchy-eliza: explicit exports. Top-level class/const declarations are not
// visible to QML importers (or Node's vm), only `var` and `function` are.
var api = {
    Eliza: Eliza, Tracer: Tracer, nullTracer: nullTracer, preTracer: preTracer,
    readScript: readScript, scriptToString: scriptToString, elizaUppercase: elizaUppercase,
    elizaTest: elizaTest, VERSION: VERSION, Tokenizer: Tokenizer,
    CACM_1966_CONVERSATION: CACM_1966_CONVERSATION
};
